...
import useUploadQuizQuestions from "../../hooks/mutations/exams/useUploadQuizQuestions";

type UploadBulkQuizQuestionsModalProps = {
  isOpen: boolean;
  toggle: VoidFunction;
...
export default function UploadQuizQuestionsModal({
  isOpen,
  toggle,
  refetch,
}: UploadBulkQuizQuestionsModalProps) {
  ...
  const { isLoading, mutate } = useUploadQuizQuestions();
          ...
          e?.data?.data?.results?.map((e: any) => {
            if (!e?.success) {
              toast.error(
                <ToastContent
                  type={"error"}
                  heading={"Error Adding Quiz Questions"}
                  message={e?.error}
                />
              );
            } else {
              toast.success(
                <ToastContent
                  type={"success"}
                  heading={"Quiz Questions added"}
                  message={"The Quiz Questions has been added successfully"}
                />
              );
              refetch();
            }
  ...
  return (
    <Modal centered {...{ isOpen, toggle }}>
      <ModalHeader toggle={toggle}> Upload Quiz Questions </ModalHeader>
      <ModalBody>
        <form onSubmit={() => {}}>
